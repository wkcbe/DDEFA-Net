...
class PyramidVisionTransformerV2(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            x, H, W = patch_embed(x)  # x(B,N,C):{12,(4096,1024,256,64),(64,128,320,512)},HW:(64,32,16,8)
            for blk in block:
                x = blk(x, H, W)  # x:{12,(4096,1024,256,64),(64,128,320,512)}
            x = norm(x)
            # x(N,C,H,W):{12,(64,128,320,512),(64,32,16,8),(64,32,16,8)}
            x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            outs.append(x)

        return outs  # 一个列表，包含每个stage的输出
